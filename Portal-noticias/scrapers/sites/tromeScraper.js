...
class TromeScraper extends BaseScraper {
  constructor(categoryId) {
    super({
      name: 'Diario Trome',
      baseUrl: 'https://trome.pe',
      categoryId: categoryId,
      headers: {
        Referer: 'https://trome.pe',
      },
    });
  }
  ...
  async scrape() {
    ...
    try {
      ...
      if (!html) {
        console.log('❌ No se pudo obtener HTML de Trome');
        return [];
      }
      ...
      for (const article of articles) {
        try {
          ...
          if (!content || content.length < 100) {
            content = article.summary || 'Contenido no disponible. Ver noticia completa en trome.pe';
          }
