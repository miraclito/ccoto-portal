...
class LaRazonScraper extends BaseScraper {
  constructor(categoryId) {
    super({
      name: 'La Razón',
      baseUrl: 'https://larazon.pe',
      categoryId: categoryId,
      headers: {
        Referer: 'https://larazon.pe',
      },
    });
  }
  ...
  async scrape() {
    ...
    try {
      ...
      if (!html) {
        console.log('❌ No se pudo obtener HTML de La Razón');
        return [];
      }
      ...
      for (const article of articles) {
        try {
          ...
          if (!content || content.length < 100) {
            content = article.summary || 'Contenido no disponible. Ver noticia completa en larazon.pe';
          }
