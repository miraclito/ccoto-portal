...
class DeporScraper extends BaseScraper {
  constructor(categoryId) {
    super({
      name: 'Diario Depor',
      baseUrl: 'https://depor.com',
      categoryId: categoryId,
      headers: {
        Referer: 'https://depor.com',
      },
    });
  }
  ...
  async scrape() {
    ...
    try {
      ...
      if (!html) {
        console.log('❌ No se pudo obtener HTML de Depor');
        return [];
      }
      ...
      for (const article of articles) {
        try {
          ...
          if (!content || content.length < 100) {
            content = article.summary || 'Contenido no disponible. Ver noticia completa en depor.com';
          }
